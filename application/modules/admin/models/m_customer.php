...
//        $sql = "select name from ship_method where id = (select method_ship_id from shipment where id = '".$id."')";
    ...
    public function getOrderByCustomerId($customer_id=""){
        $sql="SELECT order_id,order_box,order_ibs,order_sum_price, order_status_name,order_ship_charge,order_insurance_fee,order_other_charge,order_packing_fee,order_message,
            type_payment_name, type_ship_name,
            consignees.consignee_id,consignee_address,consignee_country,consignee_email,consignee_name,consignee_phone,consignee_zipcode,orders.customer_id
        FROM (((orders INNER JOIN consignees ON orders.consignee_id=consignees.consignee_id)
                LEFT JOIN order_status ON orders.order_status_id=order_status.order_status_id)
                INNER JOIN type_payment ON orders.type_payment_id=type_payment.type_payment_id)
                INNER JOIN type_ship ON orders.type_ship_id=type_ship.type_ship_id
        WHERE orders.customer_id=?";
        return $this->db->query($sql,array($customer_id))->result_array();
    }
    public function getDesciptionForOrder($customer_id=""){
        $sql="SELECT orders.order_id,order_detail_description,order_detail_quantity
        FROM order_detail INNER JOIN orders ON order_detail.order_id=orders.order_id
        WHERE orders.customer_id=?";
        return $this->db->query($sql,array($customer_id))->result_array();
    }
    //GET CONSIGNEE COUNTRY
    public function getConsigneeCountry($consignee_id=""){
        $this->db->select('consignee_country');
        $this->db->where('consignee_id',$consignee_id);
        return $this->db->get('consignees')->row_array();
    }
    //GET CONSIGNEE INFO
    public function getConsigneeInfo($consignee_id="",$consignee_country="vn"){
        $sql="SELECT consignee_id,consignee_name,consignee_phone,consignee_address,consignee_address2,consignee_country,consignee_zipcode,consignee_date,consignee_email,country_name,
            zone_cities_".$consignee_country.".city,zone_states_".$consignee_country.".state
        FROM ((consignees LEFT JOIN zone_cities_".$consignee_country." ON consignees.consignee_city=zone_cities_".$consignee_country.".id)
            LEFT JOIN zone_states_".$consignee_country." ON consignees.consignee_state=zone_states_".$consignee_country.".state_code)
            LEFT JOIN zone_countries ON consignees.consignee_country=zone_countries.country_code
        WHERE consignee_id=?";
