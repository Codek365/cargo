...
class Report extends Admin_Controller{
    ...
    public function dataselect(){
        ...
        $this->_data["all"] = $this->m_order_report->listordera_alluser_today4($start,$end,$selected);//dangtest
        // $all = $this->m_order_report->listordera_alluser_today4($start,$end,$selected);//dangtest
        $this->_data["all_user_fromto"] = $this->m_order_report->listordera_alluser_fromto4($start,$end,$selected);//dangtest
        }
        $this->load->view("report/datafromto",$this->_data);
    }
     ...
     public function detail($user_id,$order_date){
        ...
        // echo $startdaycali;
        $this->_data["link_back"]=base_url()."admin/report/index2";
        $this->_data["today"]=date('F d.Y',$now);
        
        $user_day = $this->m_order_report->user_day($startdaycali,$user_id);//dùng để kiểm tra tồn tại record hay khong http://localhost/cargo/admin/report/detail/'3'/1411423200
        if($user_day != FALSE){
        ...
        ELSE{
           $this->_data['view']='report/error_detail';
            $this->load->view($this->_data['template'], $this->_data);
        }
     }
}
