...
class Order extends Admin_Controller{
	...
	public function edit(){
		...
		if($this->input->post('save')!=""){
				...
				if($this->form_validation->run()){
					...
					if(!empty($order_id)){
						...
						if(!empty($order_id)){
							$order_status=$this->m_order->checkOrderStatus($order_id);

							if(empty($order_status['order_status_id'])){
		...
		$user_id_ses=trim($this->session->userdata('user_id'));
		if($user_id_ses!=1){
			if(empty($_GET['flag'])){
				$user_id_db=trim($this->_data['info']['user_id']);
				if($user_id_db!=$user_id_ses){
					redirect(base_url().'admin/order/index');
					exit();
				}
			}
	...
	public function quickadd(){
    	...
    	$data_insert['order_creation_date']=local_to_gmt(now());

		$order_id=$this->m_order->getTrackingCode($data_insert);

		$this->m_order->updateSha1ForOrder($order_id);
